...
In your Zementis server instance navigate to `Score/Classify Data`. Select your credit risk model from the model list. Next, upload `loan_data_new.csv` which is part of the `zementisTutorials` package. Execute the code in the console below which will return the path to the CSV file on your disk:

```{r path-to-loan-data, exercise=TRUE}

system.file("tutorials/zementis_introduction/data", package = "zementisTutorials")
```
...
Execute the code in the console below which will return the path to the Excel file on your disk:

```{r path_to_excel_file, exercise=TRUE}
