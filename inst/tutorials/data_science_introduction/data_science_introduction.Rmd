---
title: "Demystifying Data Science - Training Exercises"
tutorial:
  version: 2.2
...
# Create functions and variables for specfic exercise chapters -----------------


# NOTE: Originally, the code below was part of a setup chunk that was 
# shared by several exercises in the "Modeling Data", "Evaluatung Models" and
# "Export Model" section. However, each exercise chunk which referenced 
# the particular setup chunk always ran the entire code again which took too much time. 
# Therefore, we moved the respective code block to the global environment with 
# the drawback that a lot of variables are available to the end user via ls()
# right from the start. But this is by far the best solution since "exercise 
# chaining" (https://github.com/rstudio/learnr/issues/72#issuecomment-304995705)
# is not yet supported by learnr.


# We will use 5 bins for emp_length and 5 bins for int_rate
loan_data <- loan_data_raw %>%
  dplyr::mutate(
    emp_bin = as.character(ggplot2::cut_number(emp_length, 5)),
    int_bin = as.character(ggplot2::cut_number(int_rate, 5))
  ) %>%
  tidyr::replace_na(list(emp_bin = "Missing", int_bin = "Missing")) %>%
  dplyr::mutate(
    emp_bin = as.factor(emp_bin),
    int_bin = as.factor(int_bin),
    loan_status = as.factor(loan_status)
  ) %>%
  dplyr::select(-emp_length, -int_rate)

# Create training set (70%) and test set (30%)
set.seed(42)
index <- caret::createDataPartition(loan_data$loan_status, p = 0.7, list = FALSE)
training_set <- loan_data[index, ]
test_set <- loan_data[-index, ]

rpart_mod <- purrr::partial(rpart::rpart, 
                     # Using priors
                     parms = list(prior = c(0.7, 0.3)),
                     control = rpart::rpart.control(cp = 0.001))

# Building forest using downsampling
randomForest_mod <- purrr::partial(randomForest::randomForest, 
...
prune_tree <- function(tree) {
  cv_error_min <- which.min(tree$cptable[, "xerror"])
  cp_min <- tree$cptable[cv_error_min, "CP"]
  rpart::prune(tree, cp_min)
}
...
First things first: Welcome to the R console! R comes with a command line interface whose input is interpreted right away. The command line interpreter can even be used as a simple calculator.

Try it out yourself and type simple commands like `2+3` or `4*4` in the console below. Press `Run Code` afterwards and check out the interpreter's answer.

```{r basic-clac, exercise=TRUE}
...
You can assign single values or the result of a calculation/function call to a variable using the assignment operator `<-`. Yes, your eyes didn't trick you: It is really `<-` and not `=` in R.

Just typing the variable name at the console (plus pressing `Run Code`) will print the variable's content to the console.

Give it a try and click on `Run Code` in the console below which will execute the entire code block. Play around with the variable assignemnts and the printing a little bit by inserting different values or by even adding new variables.
...
If you like to assign a data frame column's content to a new variable you have to type the following: `new_vector <- my_data_frame$column_name`. 

In the console below we give you a small example. Go ahead and execute the code. Afterwards, assign the the cylinder column to a new variable `my_cyl` and print it to the console. 
...
parse_sw_people <- function(x) {
  x %>% httr::content(as = "text", encoding = "UTF-8") %>% 
    jsonlite::fromJSON() %>% 
    .[["results"]] %>% 
    .[, 1:8] %>% 
    dplyr::mutate(
...
The code below will download and parse some Star Wars character related data from the web. Execute it and inspect the `people` variable afterwards simply by typing `people` in the console. 
...
question("Information on how many Star Wars characters is stored in the people data frame?",
    answer("9"),
    answer("18"),
...
As you might remember from the lecture, a histogram is built by creating intervals ("bins") of a fixed length and then  by counting how many values fall into each interval. The shape of the histogram, however, is strongly dependent on the selected bin length.

Try it out yourself with the slider below. By choosing a different bin width you automatically modify the shape of the histogram.

```{r plot-histogram,  echo=FALSE}

sliderInput("bins", "Bin width:", min = 1, max = 20, value = 5)
plotOutput("histPlot")
```

```{r create-histogram, context="server"}

output$histPlot <- renderPlot(
  ggplot(aes(age), data = loan_data_raw) + 
    geom_histogram(color = "white", fill = "red", binwidth = input$bins) +
    theme(
...
The take home message here is the following: Even when doing simple plots, you need to carefully examine different plot options to gain a better understanding about the underlying data and patterns in the data.
...
question("Do you spot a difference in loan amount variation across people with different bureau scores?",
         answer("Yes, there is a difference. People with a higher bureau score who are less creditworthy tend to apply for higher loan amounts.", correct = TRUE),
...
For building trees in R you use the `rpart()` function. Since `rpart()` comes with a lot of arguments which might be a bit overwhelming for beginners you will use the function `rpart_mod()`. It is just a modified version of `rpart()` we created for you with pre-filled arguments. Therefore, you just need to specify the first two arguments and pass them to `rpart_mod`. 

The **first argument** is a formula specifying the response (dependent variable) and the predictor variables (independent variables) which should go into the model building. The so called formula argument is often used as the primary argument in functions for building ML models and has the following general form: *formula = response variable ~ predictor variables*. The "*~*" sign is read as *"is modeled as function of"*. Our response is `loan_status` and we would like to use all remaining variables as independent variables. Instead of specifying all remaining variables individually, you can use the handy shortcut "*.*" for saying *"all other variables"*. Therefore, the first argument looks as follows `loan_status ~ .` and is read as *"loan status modeled as a function of all other variables"*.

**The second argument** is the data the model should use for training: `data = training_set`.

Now it is your turn: Create your first classification tree and assign it to a new variable `my_tree`.

```{r model-tree, exercise=TRUE}
...
```{r prepare-plot-forest}

library(randomForest)
```

```{r plot-forest, exercise=TRUE, exercise.setup="prepare-plot-forest"}
...
```{r pmml, exercise = TRUE, exercise.setup="prepare-pmml"}
...
```{r pmml-export, exercise=TRUE, exercise.setup="prepare-pmml"}

dir.create(file.path(path.expand("~"), "DDS_course_export"))

saveXML(tree_pmml, file = file.path(path.expand('~'), 
                                    "DDS_course_export", "credit_risk_classification_tree.xml"))
```

**Note**: Normally, it would have been OK to specify the second argument to `saveXML()` like this in your RStudio session: `file = "./DDS_course_export/credit_risk_classification_tree.xml`. However, each of the little consoles you used throughout the course runs in its own mini R process with a separate working directory somewhere in the temp folder hierarchy of your operating system. Therefore, we stored the XML file in the `DDS_course_export` folder in your home directory. We will reference this folder also in the next course.


#### Exercise 3

In RStudio click on `File` in the upper left corner and choose `Open File...`. Navigate to `~/DDS_course_export/` and open `credit_risk_classification_tree.xml`. Verify that the PMML file was created and check out its time stamp. Click on it so that it opens in the source pane.
