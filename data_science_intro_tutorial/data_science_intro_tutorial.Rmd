...
question("Information of how many characters is stored in the people data frame?",
    answer("9"),
    answer("18"),
...
Fortunately, the math operators in R look exactly the same as in the formula above. Execute the code below and check out the `sw_bmi` vector. 
...
The command above calculated the BMI for each character based on the two columns `mass` and `height`. This command worked because R most functions are "vectorized".
...
In the console below type `people$bmi <- sw_bmi` and inspect the entire data fame `people`. Can you explain what the commands you just executed did?
