...
Go ahead create both kind of density plots (one plot with 7 lines, 7 subplot with a single line each) using the console below.
...
In the lecuture you discussed various ways how to deal with missing values in your data. In the following exercises we will cover an other topic: outliers.
...
question("What can you say about loan_data?",
         ...
         answer("The new data frame loan_data has one observation less as explected.", correct = TRUE)
)
...
rpart_mod <- partial(rpart, 
                     # Specifying a loss matrix to cope with unbalanced set
...
We already split the original data set `loan_data` in a training set and test set for you. Both are available within the consoles now as `training_set` and `test_set`. The training set consists of 70% of the original data and the test set of 30% respectively. In the first exercise you will train a classificaton tree using the training set. 
...
Now it is your turn: Create your first classificaton tree and assign it to a new variable `my_tree`.
...
Next, you will build a random forest using the training data. 
