...
Below we would like to investigate the relationship between `grade` and `home_ownership`. Complete the command accordingly by adding the `home_ownership` variable to the `table()` function. 
...
question("How many applicants house owners and had a credit score of 'D' when applying for their loan?",
              answer("434"),
              answer("1884"),
...
Now that you know the overall proportions of `loan_status` you might ask yourself the question if these proportions follow the same pattern across all bureau score classes (`grade` variable). Usually, you would expect that people with higher bureau scores default less often than people with a worse score.

Like with `table()` you can use `prop.table()` with more than one variable: `prop.table(table(vector1, vector2), margin = 1)`. In the console below please investigate the proportions of `loan_status` across the various levels of `grade`. Use `grade` as vector 1 and `loan_status` as vector 2 together with the `margin = 1` argument. 
...
The first variable we will examine a bit closer is `age`. Use the console below to create a histogram for the `age` variable. Check out the hint button should you struggle to create the plot.
...
To do so, please use `seq_along(age)` as x-axis parameter (don't forget about the y-axis parameter) and `geom = "point"` in the `qplot()` call.
...
Go ahead and create the histogram of the previous exercise. However, this time add `fill = as.factor(loan_status)` as the last argument to the `qplot()` call.
...
Create a density plot for the `loan_amnt` variable using `loan_status` as the group variable. Just re-use the command of the previous exercise. Instead of using `geom = "histogram"` please specify `geom = "density"` and map `as.factor(loan_status)` to `colour` instead to `fill`.
...
question("Do you spot an obvious difference in variation of loan_amnt between poeple who defaulted and who did not?",
         ...
         answer("No, both density plots nearly have the exact same distribution. The amount of loan does not seem to be a strong indicator to separate those two groups.", correct = TRUE)
)
...
Go ahead and create both kind of density plots (one plot with 7 lines, 7 subplots with a single line each) using the console below.
...
```{r eda-qplot-loan_amnt-four-hint}
...
question("Do you spot a difference in loan amount variation across people with different bureau scores?",
         answer("Yes, there is a difference. People with a higher bureau score who are less creditworthy tend to apply for higher credits.", correct = TRUE),
         answer("No, there is no obvious difference in loan amount variation across the groups."),
...
It is very important to highlight that all steps conducted in this phase must be properly recorded because also new data needs to follow these pre-processing steps later in any kind of production setup.
...
In the lecture you discussed various ways how to deal with missing values in your data. In the following exercises we will cover an other topic: outliers.
...
Please do the following in the console below:

* Use `dim()` to check for the dimension of `loan_data_raw` again
* Subset `loan_data_raw` by filtering for all persons younger than 100 years AND assign the results to a new variable called `loan_data`. The beginning of this call should look like `loan_data <- subset(loan_data_raw, ...`
* Use `dim()` on `loan_data`
...
Therefore, you just need to specify two arguments and pass them to `rpart_mod`. The first argument is a formula specifying the response (dependent variable) and the independent variables which should go into the model. Our response is `loan_status` and we would like to use all remaining variables as independent variables. Therefore **the first argument** looks as follows: `loan_status ~ .` "**.**" is just a shortcut for saying "all other variables". **The second argument** is the data the model should use for training: `data = training_set`.
...
question("Congrats! You just created your first machine learning model in R. But what IS this classification tree acutally?",
...
A popular method for summarizing the results is called confusion matrix. Is a contingency table of correct and incorrect classifications. It tells use how "confused" the model is with the data. 
...
In this last exercise of the course we just show you how to convert the an R ML model (here: `my_tree`) into PMML which is super easy: You just need to put your ML model into the `pmml()` function and store the results in a new variable. 

Afterwards you can save the results on disk using the `saveXML()` function. Since these are are server-based exercises we just will convert the model into PMML without storing the results to a file. 

Please execute the code below and take a look at the `tree_pmml` variable. The additional two arguments `model.name` and `description` we pass to `pmml()` are optional but good practice.

```{r pmml, exercise = TRUE}
